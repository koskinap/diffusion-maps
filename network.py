...
def main():
	...
	for bactName in sheetNames:
		...
		# sigma = np.std(d)
		# sigma = sqrt(np.median(d)**2/1.4)
		sigma  = np.median(d)- 1.5*np.std(d)
		...
		everyweak = [(u,v) for (u,v,d) in G.edges(data=True) if d['weight'] <= 0.1]

		print len(estrong),len(eweak), len(everyweak)

		pos=nx.spring_layout(G) # positions for all nodes

		# nodes
		nx.draw_networkx_nodes(G, pos, node_size=200, color = 'green')
